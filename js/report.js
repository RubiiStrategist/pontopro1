    ...
    <div class="print-card" style="padding:18px">
      ...
      <div style="display:flex;gap:12px;flex-wrap:wrap;margin:10px 0 14px 0">
        ...
        <div style="min-width:240px">
          ...
          <div>Salário base: <span class="mono">${fmtBRL(emp.monthlySalary||0)}</span></div>
<div>Trabalhadas: <span class="mono">${fmtMin(totals.worked)}</span></div>
          ...
          <div>Extras: <span class="mono">${fmtMin(totals.extra)}</span></div>
<div>Valor extras: <span class="mono">${fmtBRL(totals.valueExtra)}</span></div>
<div>Prévia (salário + extras): <span class="mono">${fmtBRL(totals.pay)}</span></div>
        </div>
      </div>
