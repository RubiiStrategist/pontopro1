...
export function calcMonthTotals(emp, monthMap){
  let worked=0, extra=0, faltas=0, folgas=0, incompleto=0, atestados=0;
  ...
  const valueExtra = (extra/60) * (emp.hourlyRate || 0);

  let pay = 0;
  if(emp.payType==="HORA"){
    pay = valueHours;
  }else{
    pay = (emp.monthlySalary || 0) + valueExtra;
  }

  return { worked, extra, faltas, folgas, incompleto, atestados, pay, valueHours, valueExtra };
}
